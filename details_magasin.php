	...
	<body class="sb-page-header">
		<div class="container">
			...
			<div id= "block1" class="container">
				<center><h3 class="	text-primary text-center ">Tous les produits de <?php $_GET['name'] ?></h3></center>
			</div>
			<div class="contenu">
	<?php
		$connexion= new PDO ('mysql:host=localhost; dbname=Shop','root','');
				if(empty($connexion)){
					...
					$produit = array();  
					$magasin =  $_GET['id'];       
					$req= $connexion -> query("SELECT * FROM produits WHERE p_id IN (SELECT p_produit_id FROM prodmag WHERE p_magasin_id= '$magasin')");
